...
class Detail_city(models.Model):
    ...
    latitude = models.FloatField(max_length=200, null=True, verbose_name='LATITUDE')
    longitude = models.FloatField(max_length=200, null=True, verbose_name='LONGITUDE')
    ...
    class Meta: # Content에 대해서 추가정보를 관리하는 정보를 메타정보라고 함
        verbose_name = 'detail_city' # 단수
        verbose_name_plural = 'detail_cities' # 복수
        db_table = 'detail_city' # 테이블명 재정의
    ...
    def get_absolute_url(self): # 현재 데이터의 절대 경로 추출
        return reverse('detail_city:detail', args=(self.city_index,))

    def get_absolute_index_url(self):
        return reverse('detail_city:detail', args=(self.slug,))
...
class Detail_cityAttachFile(models.Model):
    detail_city = models.ForeignKey(Detail_city, on_delete=models.CASCADE, related_name="files", verbose_name='Detail_city', blank=True, null=True)
    ...
    class Meta: # Content에 대해서 추가정보를 관리하는 정보를 메타정보라고 함
        ...
        db_table = 'detail_cityattachfile' # 테이블명 재정의
