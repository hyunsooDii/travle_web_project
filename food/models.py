...
class Food(models.Model):
    ...
    class Meta: # Content에 대해서 추가정보를 관리하는 정보를 메타정보라고 함
        verbose_name = 'food' # 단수
        verbose_name_plural = 'foods' # 복수
        db_table = 'food' # 테이블명 재정의
...
class FoodAttachFile(models.Model):
    food = models.ForeignKey(Food, on_delete=models.CASCADE, related_name="files", verbose_name='Food', blank=True, null=True)
    ...
    class Meta: # Content에 대해서 추가정보를 관리하는 정보를 메타정보라고 함
        ...
        db_table = 'foodattachfile' # 테이블명 재정의
